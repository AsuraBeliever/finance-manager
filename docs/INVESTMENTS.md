...
Interés compuesto diario, convención **ACT/360**: Nu publica una tasa anual y la
devenga diario sobre un año de 360 días (convención bancaria mexicana). El
rendimiento del día se calcula en la madrugada sobre el saldo de cierre del día
anterior y se capitaliza, así que el dinero depositado gana desde ese mismo día.

```rust
let r = annual_rate_bps as f64 / 10_000.0;
let days = (as_of - inv.start_date).num_days().max(0);
let value = inv.principal_cents as f64 * (1.0 + r / 360.0).powi(days as i32);
value.round() as i64
```

Como el año calendario tiene 365 días y la base son 360, el rendimiento efectivo
queda por encima del nominal: 13.00% anual compuesto diario rinde 13.88% en 360
días. Ver el ADR de 2026-08-21 en `docs/DECISIONS.md`.
...
- `daily`: `(1 + r/365)^días`. **A propósito ACT/365**, a diferencia de `nu_cajita`: aquí la tasa la teclea el usuario para cualquier producto. Para productos de banca mexicana (cajitas Nu, carteras con rendimiento) usar `nu_cajita`, que va sobre base 360.
