...
- [ ] **M1 — Capa de datos (`v0.2.0`)** `feat/db`: rusqlite (bundled), migraciones embebidas, seeds (monedas, categorías), modelos, `AppError`, comandos de lectura básicos.
- [ ] **M2 — Carteras (`v0.3.0`)** `feat/wallets`: CRUD completo + página Carteras con formulario modal y archivado.
- [ ] **M3 — Transacciones (`v0.4.0`)** `feat/transactions`: ingreso/gasto/transferencia (2 piernas atómicas), detalle de cartera con historial, filtros, borrado en par.
