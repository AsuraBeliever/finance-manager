...
export function OutboxPanel() {
  ...
  return (
    <section className="mb-4 rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
      ...
      <p className="mt-1 text-xs text-stone-500">{es.offline.pendingHint}</p>
          ...
          <li key={item.id} className="flex items-center gap-3 py-2 text-sm">
            ...
            {item.status === "error" && (
              <button
                ...
                className="rounded-md p-1.5 text-stone-400 transition-colors hover:bg-surface-overlay hover:text-stone-200"
              >
                <RotateCw size={15} />
              </button>
            )}
            <button
              ...
              className="rounded-md p-1.5 text-stone-400 transition-colors hover:bg-danger/10 hover:text-danger"
            >
              <Trash2 size={15} />
            </button>
          </li>
