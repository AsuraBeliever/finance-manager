...
// .AppImage with its own icon is what users expect. On Android it is the native
// APK. The installers are built and published to GitHub Releases by
// .github/workflows/release.yml on every tag, so here we just resolve the latest
// release's asset for the visitor's OS.

const REPO = "AsuraBeliever/finance-manager";
const RELEASES_PAGE = `https://github.com/${REPO}/releases/latest`;
const LATEST_API = `https://api.github.com/repos/${REPO}/releases/latest`;

/** OSes we ship native installers for. */
export type DownloadOs = "windows" | "linux" | "android";

/** Coarse OS family from the UA. iOS and macOS return null: those keep the PWA
 *  flow, since we don't build Apple installers in CI. */
export function detectDownloadOs(): DownloadOs | null {
...
function pickAsset(os: DownloadOs, assets: GhAsset[]): string | null {
  const find = (re: RegExp) =>
    assets.find((a) => re.test(a.name))?.browser_download_url ?? null;
  if (os === "android") return find(/_android\.apk$/i);
...
/** Resolves a one-click download for the current OS, falling back to the
