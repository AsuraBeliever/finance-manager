...
use super::{
    param_i64, param_i64_or, parse_params, parse_start_date, position_value, CalcContext,
...
mod tests {
    ...
    fn gross_value_at_maturity_act_360() {
        ...
        assert_eq!(Cetes.value_at(&inv, &ctx, maturity).unwrap(), 1_027_300);
    ...
    fn isr_retention_prorates_act_365() {
        ...
        assert_eq!(Cetes.value_at(&inv, &ctx, maturity).unwrap(), 1_026_053);
    }
    ...
    fn value_is_flat_after_maturity_and_principal_before_start() {
        ...
        assert_eq!(Cetes.value_at(&inv, &ctx, way_later).unwrap(), 1_027_300);
        let before = NaiveDate::from_ymd_opt(2025, 12, 1).unwrap();
        assert_eq!(Cetes.value_at(&inv, &ctx, before).unwrap(), 1_000_000);
    }
    ...
    fn rejects_invalid_plazo() {
        ...
        assert!(Cetes.value_at(&inv, &ctx, date).is_err());
    }
}
